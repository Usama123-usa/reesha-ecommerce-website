    ...
    try {
      ...
      if (selectedFiles.length > 0) {
        const uploadPromises = selectedFiles.map(async (file) => {
          const imageUrl = await compressAndUpload(file, productId);
  ...
  return (
    <div className="flex bg-surface text-on-background font-body min-h-screen overflow-hidden">
      ...
      <main className="flex-1 overflow-y-auto relative bg-background">
        ...
        <div className="p-margin-desktop max-w-7xl mx-auto space-y-gutter pb-24">
                  ...
                  <tr key={product.id} className="hover:bg-primary-container/5 group transition-colors">
                    <td className="px-8 py-4 flex items-center gap-4">
                      <div className="w-12 h-12 rounded-lg overflow-hidden border border-outline-variant/20">
                        <img src={product.product_images?.[0]?.image_url || 'https://via.placeholder.com/150'} alt="" className="w-full h-full object-cover" />
                      </div>
